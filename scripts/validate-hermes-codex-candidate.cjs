...
assert(candidateProfile.state === 'technical_reference', 'candidate profile must declare Hermes as a technical reference')
...
assert(read('README_OPL.md').includes('State: `retained_reference_technical_verification_official_baseline_first`'), 'README_OPL.md must preserve the retained-reference lifecycle state')
...
assert(read('docs/opl-hermes-upstream-strategy.md').includes('State: `retained_reference_guidance`'), 'upstream strategy doc must preserve retained-reference guidance')
...
assert(topologyPolicy.foreground_alternative === 'OPL Native Workbench/opl-native-workbench', 'candidate profile must preserve OPL Native Workbench as the only foreground alternative')
assert(topologyPolicy.retained_reference === 'Hermes Desktop/hermes-codex', 'candidate profile must declare Hermes as the retained reference')
...
assert(convergenceProfile.state === 'retained_reference_technical_verification_not_active_shell', 'convergence readback must retain Hermes as a non-active technical reference')
...
assert(operatorSurfaceProfile.state === 'retained_reference_operator_surface_available_not_active_shell', 'operator readback must retain Hermes as a non-active technical reference')
...
assert(falseReadyBoundary.candidate_valid_can_claim_foreground_alternative === false, 'candidate validation cannot claim foreground-alternative status')
...
assert(authorityBoundary.can_claim_foreground_alternative === false, 'candidate profile cannot claim foreground-alternative status')

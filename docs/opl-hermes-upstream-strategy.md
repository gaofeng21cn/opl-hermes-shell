...
- 本仓 OPL delta：branding、icon、bundle identity、Codex CLI adapter、
  candidate packaging wrapper 和最小验证脚本。
...
- 后端入口从 Hermes Agent 适配到 Codex CLI；未来可在明确产品决策后开放
  Codex CLI 与 Hermes Agent 双选，但普通 OPL 路径默认仍以 Codex executor 为核心。
...
Full runtime 或 WebUI parity 搬进来。
...
- **Branding/package identity**：产品名、bundle id、protocol scheme、图标、候选
  manifest。
- **Codex CLI adapter**：`electron/opl-codex-gateway.cjs` 提供 Hermes 期望的
  session/prompt JSON-RPC 形状，并把 prompt 交给 Codex CLI。
- **Electron candidate wiring**：在不接管 Hermes runtime authority 的前提下，让
  main process 能启动 OPL Codex gateway。
- **Candidate package wrapper**：生成本地 `.app` 候选包和 manifest，只用于显式
  technical verification。
- **最小验证**：证明 upstream 基线仍在、OPL branding/adapter/package wrapper 存在，
  且旧稳定线迁移文件没有被误带入。

这些改动可以作为长期 OPL delta 存在，但应保持薄、可比较、可撤回。
...
- 该能力的 source of truth 在 App repo、OPL Framework、domain agent、Codex CLI、
  Hermes upstream，还是本候选 repo。
...
- Web server 负责连接 Codex CLI、OPL CLI、workspace volume、file/preview APIs 和
  event stream；浏览器不直接拥有 runtime truth 或宿主机文件系统 authority。

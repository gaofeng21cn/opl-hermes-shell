...
function createLinkTitleFetcher({ app, BrowserWindow, session, hiddenWindowsChildOptions = options => options } = {}) {
  ...
  return function fetchLinkTitle(rawUrl) {
    const url = String(rawUrl || '').trim()
